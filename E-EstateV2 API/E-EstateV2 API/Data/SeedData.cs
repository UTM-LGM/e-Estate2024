...
namespace E_EstateV2_API.Data
{
    public class SeedData
    {
        ...
        private static async Task AddPlantingMaterial(ApplicationDbContext context)
        {
            ...
            {
                context.plantingMaterials.Add(new PlantingMaterial { plantingMaterial = "OWN NURSERY", isActive = true });
            }

            if (!context.plantingMaterials.Any(ps => ps.plantingMaterial == "OTHER NURSERY"))
            {
                context.plantingMaterials.Add(new PlantingMaterial { plantingMaterial = "OTHER NURSERY", isActive = true });
            }
            await context.SaveChangesAsync(); // Save changes to the database
        }
        ...
        private static async Task AddMembership(ApplicationDbContext context)
        {
            ...
            {
                context.membershipTypes.Add(new MembershipType { membershipType = "MAPA", isActive = true });
            }

            if (!context.membershipTypes.Any(ps => ps.membershipType == "NON MAPA"))
            {
                context.membershipTypes.Add(new MembershipType { membershipType = "NON MAPA", isActive = true });
            }
            await context.SaveChangesAsync(); // Save changes to the database
        }
        ...
        private static async Task AddCostType(ApplicationDbContext context)
        {
            ...
            {
                context.costTypes.Add(new CostType { costType = "DIRECT COST", isActive = true });
            }

            if (!context.costTypes.Any(ps => ps.costType == "INDIRECT COST"))
            {
                context.costTypes.Add(new CostType { costType = "INDIRECT COST", isActive = true });
            }
            await context.SaveChangesAsync(); // Save changes to the database
        }
        ...
        private static async Task AddlaborType(ApplicationDbContext context)
        {
            ...
            {
                context.laborTypes.Add(new LaborType { laborType = "MANAGEMENT AND PROFESSIONAL", isActive = true });
            }

            if (!context.laborTypes.Any(ps => ps.laborType == "TECHNICAL AND ADVISORY"))
            {
                context.laborTypes.Add(new LaborType { laborType = "TECHNICAL AND ADVISORY", isActive = true });
            }

            if (!context.laborTypes.Any(ps => ps.laborType == "CLERICAL AND RELATED POSITION"))
            {
                context.laborTypes.Add(new LaborType { laborType = "CLERICAL AND RELATED POSITION", isActive = true });
            }

            if (!context.laborTypes.Any(ps => ps.laborType == "GENERAL WORKER"))
            {
                context.laborTypes.Add(new LaborType { laborType = "GENERAL WORKER", isActive = true });
            }

            if (!context.laborTypes.Any(ps => ps.laborType == "FACTORY WORKER"))
            {
                context.laborTypes.Add(new LaborType { laborType = "FACTORY WORKER", isActive = true });
            }

            if (!context.laborTypes.Any(ps => ps.laborType == "OTHERS"))
            {
                context.laborTypes.Add(new LaborType { laborType = "OTHERS", isActive = true });
            }
            await context.SaveChangesAsync(); // Save changes to the database
        }
        ...
        private static async Task AddFieldStatus(ApplicationDbContext context)
        {
            ...
            {
                context.fieldStatus.Add(new FieldStatus { fieldStatus = "TAPPED AREA", isMature=true, isActive=true });
            }

            if (!context.fieldStatus.Any(ps => ps.fieldStatus == "CONVERSION TO OTHER CROP/NON-AGRI USED"))
            {
                context.fieldStatus.Add(new FieldStatus { fieldStatus = "CONVERSION TO OTHER CROP/NON-AGRI USED", isMature = true, isActive = true });
            }

            if (!context.fieldStatus.Any(ps => ps.fieldStatus == "ABANDONED - TAPPED"))
            {
                context.fieldStatus.Add(new FieldStatus { fieldStatus = "ABANDONED - TAPPED", isMature = true, isActive = true });
            }

            if (!context.fieldStatus.Any(ps => ps.fieldStatus == "ABANDONED - UNTAPPED"))
            {
                context.fieldStatus.Add(new FieldStatus { fieldStatus = "ABANDONED - UNTAPPED", isMature = true, isActive = true });
            }

            if (!context.fieldStatus.Any(ps => ps.fieldStatus == "ACQUIRED BY GOVERNMENT"))
            {
                context.fieldStatus.Add(new FieldStatus { fieldStatus = "ACQUIRED BY GOVERNMENT", isMature = true, isActive = true });
            }

            if (!context.fieldStatus.Any(ps => ps.fieldStatus == "NEW PLANTING"))
            {
                context.fieldStatus.Add(new FieldStatus { fieldStatus = "NEW PLANTING", isMature = false, isActive = true });
            }
            if (!context.fieldStatus.Any(ps => ps.fieldStatus == "REPLANTING"))
            {
                context.fieldStatus.Add(new FieldStatus { fieldStatus = "REPLANTING", isMature = false, isActive = true });
            }
            if (!context.fieldStatus.Any(ps => ps.fieldStatus == "CONVERSION OTHER CROP TO RUBBER"))
            {
                context.fieldStatus.Add(new FieldStatus { fieldStatus = "CONVERSION OTHER CROP TO RUBBER", isMature = false, isActive = true });
            }
            if (!context.fieldStatus.Any(ps => ps.fieldStatus == "ABANDONED"))
            {
                context.fieldStatus.Add(new FieldStatus { fieldStatus = "ABANDONED", isMature = false, isActive = true });
            }
            if (!context.fieldStatus.Any(ps => ps.fieldStatus == "ACQUIRED BY GOVERNMENT"))
            {
                context.fieldStatus.Add(new FieldStatus { fieldStatus = "ACQUIRED BY GOVERNMENT", isMature = false, isActive = true });
            }
            await context.SaveChangesAsync(); // Save changes to the database
        }
        ...
        private static async Task AddCountry(ApplicationDbContext context)
        {
            ...
            {
                context.countries.Add(new Country { country = "MALAYSIA", isLocal=true, isActive = true });
            }

            if (!context.countries.Any(ps => ps.country == "INDONESIA"))
            {
                context.countries.Add(new Country { country = "INDONESIA", isLocal=false, isActive = true });
            }

            if (!context.countries.Any(ps => ps.country == "BANGLADESH"))
            {
                context.countries.Add(new Country { country = "BANGLADESH", isLocal = false, isActive = true });
            }

            if (!context.countries.Any(ps => ps.country == "INDIA"))
            {
                context.countries.Add(new Country { country = "INDIA", isLocal = false, isActive = true });
            }

            if (!context.countries.Any(ps => ps.country == "THAILAND"))
            {
                context.countries.Add(new Country { country = "THAILAND", isLocal = false, isActive = true });
            }

            if (!context.countries.Any(ps => ps.country == "NEPAL"))
            {
                context.countries.Add(new Country { country = "NEPAL", isLocal = false, isActive = true });
            }

            if (!context.countries.Any(ps => ps.country == "MYANMAR"))
            {
                context.countries.Add(new Country { country = "MYANMAR", isLocal = false, isActive = true });
            }

            if (!context.countries.Any(ps => ps.country == "VIETNAM"))
            {
                context.countries.Add(new Country { country = "VIETNAM", isLocal = false, isActive = true });
            }

            if (!context.countries.Any(ps => ps.country == "OTHERS"))
            {
                context.countries.Add(new Country { country = "OTHERS", isLocal = false, isActive = true });
            }
        ...
        private static async Task AddCostCategory(ApplicationDbContext context)
        {
            ...
            {
                context.costCategories.Add(new CostCategory { costCategory = "Upkeep and Cultivation", isActive = true });
            }

            if (!context.costCategories.Any(ps => ps.costCategory == "Tapping and Collection"))
            {
                context.costCategories.Add(new CostCategory { costCategory = "Tapping and Collection", isActive = true });
            }

            if (!context.costCategories.Any(ps => ps.costCategory == "Stimulation"))
            {
                context.costCategories.Add(new CostCategory { costCategory = "Stimulation", isActive = true });
            }

            if (!context.costCategories.Any(ps => ps.costCategory == "Transport to Factory"))
            {
                context.costCategories.Add(new CostCategory { costCategory = "Transport to Factory", isActive = true });
            }

            if (!context.costCategories.Any(ps => ps.costCategory == "N/A"))
            {
                context.costCategories.Add(new CostCategory { costCategory = "N/A", isActive = true });
            }
        ...
        private static async Task AddCostCategory1(ApplicationDbContext context)
        {
            ...
            {
                context.costSubcategories1.Add(new CostSubcategory1 { costSubcategory1 = "Manuring", isActive = true });
            }

            if (!context.costSubcategories1.Any(ps => ps.costSubcategory1 == "Weeding"))
            {
                context.costSubcategories1.Add(new CostSubcategory1 { costSubcategory1 = "Weeding", isActive = true });
            }

            if (!context.costSubcategories1.Any(ps => ps.costSubcategory1 == "Others( Pest and disease control)"))
            {
                context.costSubcategories1.Add(new CostSubcategory1 { costSubcategory1 = "Others( Pest and disease control)", isActive = true });
            }

            if (!context.costSubcategories1.Any(ps => ps.costSubcategory1 == "Tapping and Collection"))
            {
                context.costSubcategories1.Add(new CostSubcategory1 { costSubcategory1 = "Tapping and Collection", isActive = true });
            }

            if (!context.costSubcategories1.Any(ps => ps.costSubcategory1 == "Stimulation"))
            {
                context.costSubcategories1.Add(new CostSubcategory1 { costSubcategory1 = "Stimulation", isActive = true });
            }

            if (!context.costSubcategories1.Any(ps => ps.costSubcategory1 == "Transport to Factory"))
            {
                context.costSubcategories1.Add(new CostSubcategory1 { costSubcategory1 = "Transport to Factory", isActive = true });
            }

            if (!context.costSubcategories1.Any(ps => ps.costSubcategory1 == "N/A"))
            {
                context.costSubcategories1.Add(new CostSubcategory1 { costSubcategory1 = "N/A", isActive = true });
            }
        ...
        private static async Task AddCostCategory2(ApplicationDbContext context)
        {
            ...
            {
                context.costSubcategories2.Add(new CostSubcategory2 { costSubcategory2 = "Material", isActive = true });
            }

            if (!context.costSubcategories2.Any(ps => ps.costSubcategory2 == "Labour"))
            {
                context.costSubcategories2.Add(new CostSubcategory2 { costSubcategory2 = "Labour", isActive = true });
            }

            if (!context.costSubcategories2.Any(ps => ps.costSubcategory2 == "Total"))
            {
                context.costSubcategories2.Add(new CostSubcategory2 { costSubcategory2 = "Total", isActive = true });
            }
            ...
            {
                context.costSubcategories2.Add(new CostSubcategory2 { costSubcategory2 = "Clearing, planting and any direct maintenance expenditure incurred on area new planted with rubber", isActive = true });
            }
            ...
            {
                context.costSubcategories2.Add(new CostSubcategory2 { costSubcategory2 = "Direct maintenance expenditure in respect of all immature rubber area replanted/new planted in this year or earlier", isActive = true });
            }
            ...
            {
                context.costSubcategories2.Add(new CostSubcategory2 { costSubcategory2 = "Managerial and administration staff salaries, allowances and other expenses", isActive = true });
            }

            if (!context.costSubcategories2.Any(ps => ps.costSubcategory2 == "Maintenance of buildings and other permanents fixtures"))
            {
                context.costSubcategories2.Add(new CostSubcategory2 { costSubcategory2 = "Maintenance of buildings and other permanents fixtures", isActive = true });
            }

            if (!context.costSubcategories2.Any(ps => ps.costSubcategory2 == "Depreciation of buildings and other permanents fixtures"))
            {
                context.costSubcategories2.Add(new CostSubcategory2 { costSubcategory2 = "Depreciation of buildings and other permanents fixtures", isActive = true });
            }
            ...
            {
                context.costSubcategories2.Add(new CostSubcategory2 { costSubcategory2 = "Fees, quit rents, assesments, land cess, rent, insurance and other payment", isActive = true });
            }

            if (!context.costSubcategories2.Any(ps => ps.costSubcategory2 == "Labour expenses"))
            {
                context.costSubcategories2.Add(new CostSubcategory2 { costSubcategory2 = "Labour expenses", isActive = true });
            }

            if (!context.costSubcategories2.Any(ps => ps.costSubcategory2 == "Head office charges (if any)"))
            {
                context.costSubcategories2.Add(new CostSubcategory2 { costSubcategory2 = "Head office charges (if any)", isActive = true });
            }

            if (!context.costSubcategories2.Any(ps => ps.costSubcategory2 == "Others (including general office overheads)"))
            {
                context.costSubcategories2.Add(new CostSubcategory2 { costSubcategory2 = "Others (including general office overheads)", isActive = true });
            }
