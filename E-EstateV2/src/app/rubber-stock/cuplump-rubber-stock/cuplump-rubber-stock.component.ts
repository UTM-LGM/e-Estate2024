...
@Component({
  selector: 'app-cuplump-rubber-stock',
  templateUrl: './cuplump-rubber-stock.component.html',
  styleUrls: ['./cuplump-rubber-stock.component.css']
})
export class CuplumpRubberStockComponent implements OnInit, OnDestroy {
  ...
  getStock() {
    setTimeout(() => {
      const getRubberStock = this.rubberStockService.getRubberStock()
        .subscribe(
          Response => {
            this.rubberStocks = Response.filter(e => e.estateId == this.sharedService.estateId && e.rubberType == 'CUPLUMP')
            this.isLoading = false
          }
        )
